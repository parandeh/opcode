...
export const TableWidget: React.FC<TableWidgetProps> = ({ 
  ...
  const [internalIsExpanded, setInternalIsExpanded] = useState(
    controlledIsExpanded !== undefined ? controlledIsExpanded : true
  );
  
  // Only use controlled state if both isExpanded and onExpandChange are provided
  const isControlled = controlledIsExpanded !== undefined && onExpandChange !== undefined;
                ...
                <tbody>
                  {data.map((row, rowIdx) => (
                    <tr 
                      key={rowIdx} 
                      className={cn(
                        "border-b border-muted/50 hover:bg-muted/40 transition-colors cursor-pointer",
